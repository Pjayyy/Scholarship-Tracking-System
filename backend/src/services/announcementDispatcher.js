...
function buildTransport() {
  ...
  const rateLimit = Number(process.env.SMTP_RATE_LIMIT || 0);
...
async function dispatchAnnouncement({ announcementId }) {
  ...
  try {
    ...
    if (!aRows || aRows.length === 0) return { ok: false, reason: "not_found" };
    ...
    const bccMode = String(process.env.SMTP_BCC_MODE || "false").toLowerCase() === "true";
