...
function getGmailConfig() {
  return {
    ...
    query: process.env.GMAIL_POLL_QUERY?.trim() || "is:unread newer_than:60d",
...
function isGmailIngestConfigured() {
  ...
  if (!c.allowedSubstrings.length) return false;
  if (process.env.GMAIL_INGEST_ENABLED === "false") return false;
  return true;
}

function buildOAuthClient() {
  const { clientId, clientSecret, refreshToken } = getGmailConfig();
  const redirectUri = process.env.GOOGLE_OAUTH_REDIRECT_URI?.trim() || "http://127.0.0.1:49153/oauth2callback";
...
function headerValue(headers, name) {
  ...
  const row = headers.find((h) => h.name && h.name.toLowerCase() === lower);
  return row?.value || "";
}
...
async function pollGmailAndIngest() {
  ...
  for (const m of messages) {
    ...
    if (!fromAllowed(from, cfg.allowedSubstrings)) continue;

    const bodyText = extractPlainFromPayload(full.data.payload) || full.data.snippet || "";
    ...
    try {
      ...
      console.error("Announcement dispatch failed:", e?.message || e);
    }
  }
